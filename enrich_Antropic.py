...
def process_trending_data(system_definition, trending_data):
    ...
    try:
        ...
        # API-Anfrage
        GermanTrendingTopicsAnalysis_assistent = client.messages.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": json.dumps(trending_data, ensure_ascii=False)
                }
            ]
        )

        # Antwort als JSON parsen
        response_text = GermanTrendingTopicsAnalysis_assistent.content[0].text
        try:
...
def main():
    ...
    output_file = 'trending_data_processed_Antrophic.json'
